...
async def show_main_menu(client: Client, message: Message, user_id: int):
    ...
    buttons = [[KeyboardButton("⭐ Mening balansim")], [KeyboardButton("🔗 Referal havola")], [KeyboardButton("💳 Stars yechish")], [KeyboardButton("🎁 Premium olish")], [KeyboardButton("📘 Qo'llanma")]]
    if is_admin:
...
async def withdraw_stars(client: Client, message: Message):
    ...
    for amount, emoji in withdraw_amounts:
        if balance >= amount:
            buttons.append([InlineKeyboardButton(f"{emoji} {amount} ⭐", callback_data=f"withdraw_{amount}")])
        else:
            buttons.append([InlineKeyboardButton(f"{emoji} {amount} ⭐ ❌", callback_data="insufficient_balance")])
...
if __name__ == "__main__":
    from panel import register_admin_handlers
    register_admin_handlers(app, db, check_admin, show_main_menu)
